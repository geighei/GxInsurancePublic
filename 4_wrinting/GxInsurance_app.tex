...
Figure \ref{fig:cv} plots the share of people reporting a health shock since the last HRS survey over the different ages.
...
Panels (a), (b), and (c) of Figure \ref{fig:income_ret} show that there is no apparent jump in income or the probability of retirement exactly at the age of 65 in our sample.
...
As shown in Figure \ref{fig:PGS_distr}, the PGS is mildly correlated with gender and almost uncorrelated with the probability of suffering from a health shock.

\begin{figure}[ht]
	\begin{center}
		\subfigure[\textsf{PGS distribution and correlation with smoking behavior.}]{\includegraphics[width=8cm]{../3_output/make_histograms/pgs_density_6070_smoken_smooth.png}} %\label{fig:PGS}
		...
		\caption{Distribution of PGS for Regular Smoking and its relation to the main analysis varaibles in the HRS Data
			\label{fig:PGS_distr}}
	\vspace{-3ex}
	\floatfoot{
	\textit{Notes:} Distribution of Polygenic Score (PGS) for baseline smokers (blue) and non-smokers (red).
	Generalized linear smoothed correlation between current smoking (panel a), gender (panel b), and health shock (panel b) and the PGS shown in the blue line (with 95\% confidence intervals in shaded grey area).
	\\ \textit{Data used}: HRS waves 1-13, restricted to observations with age between 60 and 70 years.
	}
	\end{center}
\end{figure}
...
\begin{table*}[!ht]
	...
	%\resizebox{7.5cm}{!}{
	\input{../3_output/OLSresults/placebo_results.tex}
		\begin{flushleft}
			Low PGS = lowest tertile of the polygenic score distribution; high PGS = upper two tertiles of the polygenic score distribution.
			Pre-55: Health shock since the last survey reported at ages 50-54.
			Post-55: Health shock since the last survey reported at ages 57-60.
...
\begin{table}[!ht] \centering
	...
	\input{../3_output/OLSresults/reg_results_multi_checkmarks_Placebo.tex}
	} %end of resizebox
	\begin{flushleft}
	\textit{Notes:}
...
%
%\subsection{Limitations}\label{appsec:limitations}
%
...
%However, the fraction of eligible HRS respondents in the study sample who are actually enrolled in Medicare is high---approximately 90\% at age 65, increasing to 98\% at age 70.
